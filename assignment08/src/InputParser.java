...
public final class InputParser {
	...
	private static final String PatternName = "(?:[A-Z]([a-z])+)";
    ...
    //ASSIGNMENT 8
	public static Person parsePerson(File file) {
		BufferedReader reader;
		...
		try{
			...
			String line = reader.readLine();
			words = line.split("\\s");
            finalPerson = new Person(words[0], words[1], DateFormat.parse(words[2]));
			reader.close();
			return finalPerson;
		} catch (Throwable e) {
			System.out.println("Problem with file path!!");
	...
	public static Date deserialize(DataInputStream input) throws IOException, ParseException {
		String birthDate = input.readUTF();
